...
$scope.selectedJoinTypes = [];
$scope.selecteAggrUnit= [];
...
$scope.joinTypes = [
	{id:1, 'name':'Inner'},
	{id:2, 'name':'Outer'},
	{id:3, 'name':'Left outer'},
	{id:4, 'name':'Right outer'}
];
		  ...
		  data: { // element data (put dev data here)
			...
			conditions : [
                ...
                },
				{
				 "attribute":"type",
				 "operator":"=",
				 "value": $scope.sourceTypes[0].name
				}
			],
			foo: 1, bar: 1, baz: 8
		  },
...
function getJoinObj() {
		var join = { // node n1
		  ...
		  data: { // element data (put dev data here)
		    ...
		    faveShape : 'octagon',
			object : 'join',
      join_type : $scope.joinTypes[0].name,
			join_schema : []
		  },
		  // scratchpad data (usually temp or nonserialisable data)
		  scratch: {
...
function connectNodes() {
    ...
    if(satisfyDataflowConditions(source_node, target_node)){
       ...
       target_node.data.object == 'aggregate' ||
      ...
      if(target_node.data.object == 'aggregate'){
        ...
        else if(target_node.data.object == 'join'){
					canSetSchema = true;
				}
				else {
					canSetSchema = false;
...
function rewrite(deleted) {
	...
	if(type === 'source') {
        ...
        $scope.schemaPropertiesSelected.splice(idx, 1);
        $scope.selectedJoinTypes.splice(pos, 1);
    }
...
function getDCN(json) {
  ...
  dcnService.transformJSON(json).then(
      function(response){
        ...
        alertsManager.clearAlerts();
		alertsManager.addAlert("Spark SQL generated successfully", 'alert-success');
		  $timeout(function() {
			  $scope.showCustomAlert = false;
			  alertsManager.clearAlerts();
		  }, 3000);
...
$scope.getJSON = function(action) {
    ...
    cy.nodes().forEach(function( ele ){
      ...
      var join_type = $scope.selectedJoinTypes[data.pos];
			var ids = [];
		...
		// }
			...
			if(settings.time_unit != undefined) {
				...
				var aggregate_data_name = settings.aggregate_data_name.name.name;
				element = {
					"data":{
					   "id": data.id,
					   "name": data.name,
					   "object": data.object,
					   "attributes":attributes,
             "nds": data.schemas[0].m2m_data_schema,
					   "settings" : {
						 "aggregate_data_name": aggregate_data_name,
						 "time_interval": settings.time_interval,
						 "time_unit": settings.time_unit
    ...
    if(args.cy != "") {
		...
		if(cy.elements.hasOwnProperty('nodes')) {
              ...
              if(value.data.object === 'join'){
                ...
                //$scope.selectedJoinTypes[node.data.pos] = { name : value.data.join_type.name};
                console.log($scope.selectedJoinTypes[node.data.pos]);
...
$scope.save = function(saveDefault) {
	...
	if(json.elements.hasOwnProperty('nodes')){
        ...
        //console.log("aggDN " + $scope.selectedAggregateAttribute[data.pos].name);
				if(settings.time_unit != undefined) {
					settings.time_unit = settings.time_unit.hasOwnProperty('name') ? settings.time_unit.name : settings.time_unit;
				}
				settings_obj = {
        ...
        var join_type = $scope.selectedJoinTypes[data.pos];
        json.elements.nodes[x].data.join_type = join_type;
...
$scope.getSchemaByType = function(sensor_type){
    for(var x=0; x<$scope.schemas.length; x++){
        if($scope.schemas[x].sensor_type == sensor_type){
            ...
            console.log("get" + $scope.schemas[x]);
        }
    }
};
...
$scope.setNodeSchema = function(node, type){
	...
	if(sensor_type != false){
      ...
      console.log("idx "+node.data.id);
			var schema = cloneObject($scope.getSchemaByType(sensor_type));
      console.log("shcema"+schema+" sensor type"+sensor_type);
